...
const GroupComponent3 = ({
  ...
  const orderTrackingStyle = useMemo(() => {
    return {
      padding: propPadding,
    };
  }, [propPadding]);

  const loyaltyProgramTextStyle = useMemo(() => {
    return {
  ...
  return (
    <div className="h-[242px] flex flex-col items-center justify-start text-left text-xl text-gray-700 font-lato">
      ...
      />
      <div
        className="self-stretch flex flex-row items-start justify-start py-0 pr-1.5 pl-2"
        style={orderTrackingStyle}
      >
        <div className="flex-1 flex flex-col items-center justify-start gap-[11px]">
          <div
            className="flex flex-row items-start justify-start py-0 pr-[15px] pl-[9px]"
            style={loyaltyProgramTextStyle}
          >
            <div
